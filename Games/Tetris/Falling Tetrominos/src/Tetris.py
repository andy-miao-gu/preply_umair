...
while True:
    ...
    moving_time   = current_time - last_moving_time
    rotating_time = current_time - last_rotating_time
    ...
    if not gameOver:
        if tetromino.active:
            ...
            #Movement
            if moving_time >= movingRate[current_level-1]:            
                last_moving_time = current_time
                if pressed_keys[K_RIGHT] and move_right_ok:
                    tetromino.move(grid,1,0)
                if pressed_keys[K_LEFT] and move_left_ok:
                    tetromino.move(grid,-1,0)
                if pressed_keys[K_DOWN] and move_down_ok:
                    tetromino.move(grid,0,1)
                    currentY += 1
                if pressed_keys[K_SPACE]:
                    fallingRate -= rateIncrease
            #Rotation
            if rotating_time >= rotatingRate[current_level-1]:
                last_rotating_time = current_time
                if pressed_keys[K_UP] and move_right_ok and move_left_ok and move_down_ok:
        ...
        if grid.numLinesCleared % level_up_line_count == 0 and lastNumLinesCleared != grid.numLinesCleared:
            ...
            current_level += 1
            sound.play("/Users/andymiaogu/Desktop/preply_umair/Tetris/Falling Tetrominos/sound/levelup.wav")
        #Render Background
